...
impl IncrementalCompiler {
    ...
    fn hash_file_contents(&self, file_path: &Path) -> Result<u64> {
        let contents = fs::read_to_string(file_path)
            .map_err(|e| CompileError::codegen_error(format!("Failed to read file {}: {}", file_path.display(), e), None))?;
    ...
    fn has_file_changed(&self, file_path: &Path) -> Result<bool> {
        let metadata = fs::metadata(file_path)
            .map_err(|e| CompileError::codegen_error(format!("Failed to get metadata for {}: {}", file_path.display(), e), None))?;
    ...
    pub fn add_unit(&mut self, file_path: PathBuf, ast: Vec<Stmt>, type_context: TypeContext, compilation_time: Duration) -> Result<()> {
        let metadata = fs::metadata(&file_path)
            .map_err(|e| CompileError::codegen_error(format!("Failed to get metadata for {}: {}", file_path.display(), e), None))?;
...
pub fn get_incremental_compiler() -> &'static mut IncrementalCompiler {
    unsafe {
        GLOBAL_INCREMENTAL_COMPILER.as_mut().expect("Incremental compiler not initialized")
    }
}
...
pub fn compile_file_incremental(file_path: &Path) -> Result<(Vec<Stmt>, TypeContext)> {
    ...
    if let Some((ast, type_context)) = compiler.get_cached_result(file_path) {
        eprintln!("✅ Using cached compilation result for {}", file_path.display());
    ...
    // Cache the result
    compiler.add_unit(file_path.to_path_buf(), ast.clone(), type_context.clone(), compilation_time)?;
    compiler.stats.units_recompiled += 1;
    compiler.stats.time_spent += compilation_time;
    
    eprintln!("✅ Compiled {} in {:?}", file_path.display(), compilation_time);
...
mod tests {
    ...
    fn test_file_change_detection() {
        ...
        compiler.add_unit(file_path.clone(), ast, type_context, Duration::from_millis(100)).unwrap();
    ...
    fn test_dependency_tracking() {
        ...
        let order = compiler.get_compilation_order(&[file_a.clone(), file_b.clone(), file_c.clone()]).unwrap();
