...
fn test_simd_vector_literal_codegen() {
    ...
    let (program, _) = compile_to_ast(source).expect("SIMD vector literal should parse");
    ...
    assert!(
        ...
        "Should generate 4-element float vector"
    );
}
...
fn test_simd_integer_vector_codegen() {
    ...
    let (program, _) = compile_to_ast(source).expect("SIMD integer vector should parse");
    ...
    let ir = codegen.emit_ir();
    assert!(
        ir.contains("<4 x i32>") || ir.contains("<4 x i64>"),
        "Should generate 4-element integer vector"
    );
}
...
fn test_simd_elementwise_add_codegen() {
    ...
    let (program, _) = compile_to_ast(source).expect("SIMD elementwise add should parse");
    ...
    assert!(
        result.is_ok(),
        "SIMD elementwise add should compile to LLVM"
    );

    let ir = codegen.emit_ir();
    // Should contain vector addition instruction
    assert!(
        ir.contains("fadd") && ir.contains("<4 x float>"),
        "Should generate vector float add"
    );
}
...
fn test_simd_elementwise_multiply_codegen() {
    ...
    let (program, _) = compile_to_ast(source).expect("SIMD elementwise multiply should parse");
    ...
    assert!(
        result.is_ok(),
        "SIMD elementwise multiply should compile to LLVM"
    );

    let ir = codegen.emit_ir();
    // Should contain vector multiplication instruction
    assert!(
        ir.contains("mul") && (ir.contains("<4 x i32>") || ir.contains("<4 x i64>")),
...
fn test_simd_bitwise_operations_codegen() {
    ...
    let (program, _) = compile_to_ast(source).expect("SIMD bitwise operations should parse");
    ...
    assert!(
        result.is_ok(),
        "SIMD bitwise operations should compile to LLVM"
    );

    let ir = codegen.emit_ir();
    // Should contain bitwise operations
    assert!(
        ir.contains("and") || ir.contains("or") || ir.contains("xor"),
...
fn test_different_simd_vector_sizes() {
    ...
    let (program, _) = compile_to_ast(source).expect("Different SIMD sizes should parse");
    ...
    assert!(
        result.is_ok(),
        "Different SIMD sizes should compile to LLVM"
    );

    let ir = codegen.emit_ir();
    // Should contain different vector sizes
    assert!(
        ir.contains("<2 x float>"),
        "Should generate 2-element vector"
    );
    assert!(
        ir.contains("<4 x float>"),
        "Should generate 4-element vector"
    );
...
fn test_simd_in_function_parameters() {
    ...
    if let Err(e) = result {
        ...
        assert!(
            format!("{:?}", e).contains("type") || format!("{:?}", e).contains("parse"),
        ...
        assert!(
            ir.contains("<4 x float>"),
            "Should handle SIMD function parameters"
        );
    }
}
...
fn test_simd_complex_expression() {
    ...
    let (program, _) = compile_to_ast(source).expect("Complex SIMD expression should parse");
    ...
    assert!(
        result.is_ok(),
        "Complex SIMD expression should compile to LLVM"
    );

    let ir = codegen.emit_ir();
    // Should contain multiple vector operations
    assert!(
        ir.contains("fadd") && ir.contains("fmul"),
...
fn test_simd_compilation_integration() {
    ...
    let (program, context) = compile_to_ast(source).expect("SIMD demo should parse and type check");

    // Verify type checking worked
    assert!(
        context.functions.contains_key("simd_demo"),
        "Should have simd_demo function"
    );
    ...
    assert!(
        ir.contains("<4 x float>"),
        "Should use 4-element float vectors"
    );
    assert!(
        ir.contains("fadd") || ir.contains("fmul"),
        "Should have vector arithmetic"
    );

    println!(
        "SIMD Demo IR Preview:\n{}",
