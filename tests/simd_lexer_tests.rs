...
use ea_compiler::{Lexer, TokenKind, Position};
...
fn test_simd_function_keywords() {
    let source = "horizontal_sum horizontal_min horizontal_max from_slice to_array splat shuffle lanes";
    let mut lexer = Lexer::new(source);
    let tokens = lexer.tokenize_all().unwrap();
...
fn test_comprehensive_simd_types() {
    ...
    let simd_tokens: Vec<_> = tokens.into_iter()
...
fn test_simd_keyword_identifier_distinction() {
    ...
    assert_eq!(tokens[0].kind, TokenKind::Identifier("vectorize_func".to_string()));
    assert_eq!(tokens[1].kind, TokenKind::Vectorize);
    assert_eq!(tokens[2].kind, TokenKind::Identifier("my_simd_width".to_string()));
...
fn test_complex_simd_expressions() {
    ...
    let simd_ops: Vec<_> = tokens.iter()
...
fn test_simd_lexer_performance() {
    ...
    assert!(tokens.len() > 10000, "Should have many tokens, got {}", tokens.len());
...
fn test_simd_type_edge_cases() {
    ...
    let non_eof_tokens: Vec<_> = tokens.into_iter()
        .filter(|t| t.kind != TokenKind::Eof)
        .collect();
    
    for token in non_eof_tokens {
        assert!(matches!(token.kind, TokenKind::Identifier(_)), 
                "Token should be identifier, got {:?}", token.kind);
    }
}
